...
namespace vbm
{
    ...
    {
        public async Task<internal_contact> check_order_time(bool IsTimed, DateTime time, int ShopID)
        {
            try
            {
                string url = input_data.check_order_time_url + $"?IsTimed={IsTimed}&time={time}&ShopID={ShopID}";
